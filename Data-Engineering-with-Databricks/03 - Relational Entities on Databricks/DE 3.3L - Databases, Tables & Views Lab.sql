...
create database it not exists ${da.db_name}
...
CREATE DATABASE if not exists ${da.db_name} LOCATION '${da.paths.working_dir}/${da.db_name}';
USE ${da.db_name};
...
create or replace temporary view celcius_temp
AS (SELECT *
  FROM weather_managed
  WHERE UNIT = "C")
